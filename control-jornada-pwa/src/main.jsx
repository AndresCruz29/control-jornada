...
import {
  ...
  Undo2, ChevronDown, ChevronRight, CalendarDays,
...
const EVENT_META = {
  ...
  otro_inicio: { label: "Inicio otro", icon: CircleDot, ramp: "coral" },
  otro_fin: { label: "Fin otro", icon: CircleDot, ramp: "amber" },
};
...
function getSchedule(dt) {
  ...
  if(dow>=1 && dow<=4)
    return {start:"07:00",end:"17:00",hours:10};

  if(dow===5)
    return {start:"07:00",end:"15:00",hours:8};
...
function computeStats(events, nowTs) {
  ...
  for(const ev of sorted){
    ...
    if(lastTs!==null){
      ...
      else if(state==="otro") otherMs+=delta;
...
function getEvents(dateStr){
  try{
    return JSON.parse(
      localStorage.getItem(`timeclock:${dateStr}`)||"[]"
    );
  }catch{
    return [];
  }
...
function getIndex(){
  try{
    return JSON.parse(
      localStorage.getItem("timeclock:index")||"[]"
    );
  }catch{
    return [];
  }
...

      {
        ...
        ? <main>
          ...
          {activeLabel && (
            ...
            <section className="active-timer">
